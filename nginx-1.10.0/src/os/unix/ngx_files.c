...
/*
 * *********************文件锁*************************
 * Nginx中文件锁是通过fcntl函数调用实现的，其原型如下:
 * int fcntl(int fd, int cmd, struct flock *lock);
 * 其中fd是打开的文件句柄，cmd表示执行的锁操作，参数lock描述了这个锁的信息
 * 1. 参数fd必须是已经成功打开的文件句柄，在Nginx的配置文件nginx.conf中的lock_file
 *    指定的文件路径就是用于文件锁的
 * 2. cmd在nginx中只会有一下几个值: F_SETLK、F_SETLKW。其中F_SETLK表示以
 *    非阻塞方式获取文件锁，即如果在获取锁的时候锁被其他进程占用，则会立即返回获取锁
 *    失败，而不等待其他进程释放锁;F_SETLKW表示以阻塞方式获取锁，即如果锁被其他进程占用，
 *    fcntl方法会一直等待，直到其他进程释放锁，这会使当前进程进入睡眠状态
 * 3. lock结构体描述的是锁的信息，其结构体如下(只列举和文件锁有关的成员):
 *     struct flock{
 *         .......
 *         short l_type;    //锁类型，取值为F_RDLCK F_WRLCK F_UNLCK
 *         short l_whence;  //锁区域起始地址的相对位置
 *         long l_start;    //锁区域起始地址偏移量，和l_whence共同确定锁区域
 *         long l_len;      //锁长度，0表示锁至文件末尾
 *         long l_pid;      //拥有锁的进程id
 *         .......
 *     }
 *     在nginx中，一个lock_file文件对应一个全局互斥锁，而且它对master和worker进程
 *     都有效，因此，对于l_start,l_len,l_pid都填为0，而l_whence则填为SEEK_SET，只需要
 *     这个文件提供一个锁;l_type的值则取决于用于想实现阻塞睡眠锁还是非阻塞非睡眠锁
 *     l_type的值可以有以下三个，分别介绍如下:
 *     1) F_RDLCK:读锁(共享锁)，不同的进程可以在同一文件的同一区域有一个"共享锁"，如果
 *        一个进程在某文件的某个区域设置了共享锁，则其他进程在这个区域不能设置独占锁
 *     2) F_ERLCK:写锁(独占锁)，一个区域只允许一个进程在这里设置独占锁，此时其他进程在
 *        这里不能获取任何类型的锁
 *     3) F_UNLCK: 释放锁
 *     4) SEEK_SET 表示文件开始位置 SEEK_CUR 表示当前位置 SEEK_END 表示文件结束位置
 */

/*
 * 以非阻塞方式获取锁，获取锁成功则返回0，如果锁被其他进程占用，则返回错误码，如果错误码
 * 值为NGX_EAGIN或者NGX_EACCESS表示当前没有拿到锁，否则认为是fcntl执行错误
 */
ngx_err_t
...
/*
 * 以阻塞方式获取锁，该方法返回0表示成功获取锁，否则就是加锁操作出现的错误
 */
ngx_err_t
